...
public class CityPicker implements CanShow, OnWheelChangedListener {
     ...
     * 标题背景颜色
     */
    private int titleBackgroundColorStr = android.R.color.darker_gray;
    /**
     * 标题颜色
     */
    private int titleTextColorStr = android.R.color.darker_gray;
    ...
    private CityPicker(Builder builder) {
        ...
        if (this.titleTextColorStr!=0) {
            mTvTitle.setTextColor(ActivityCompat.getColor(context,this.titleTextColorStr));
        }
            ...
            for (int i = 0; i < provinceList.size(); i++) {
                ...
                for (int j = 0; j < cityList.size(); j++) {
                    ...
                    for (int k = 0; k < districtList.size(); k++) {
                        ...
                        mZipcodeDatasMap.put(mProvinceDatas[i] + cityNames[j] + districtList.get(k).getName(), districtList.get(k).getZipcode());
