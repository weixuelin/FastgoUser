...
import com.wt.fastgo_user.adapter.SuppliesAdapter;
...
public class SuppliesFragment extends BaseFragment {
    ...
    Unbinder unbinder;
    @BindView(R.id.recycler_supplies)
    RecyclerView recyclerSupplies;
    private ArrayList<HomeModel> arrayList;
    private SuppliesAdapter adapter;

    @Override
    protected View getSuccessView() {
        View view = View.inflate(getActivity(), R.layout.fragment_supplies, null);
    ...
    private void setListener() {
        //设置固定大小
        recyclerSupplies.setHasFixedSize(true);
        //创建线性布局
        LinearLayoutManager mLayoutManager = new LinearLayoutManager(getActivity());
        //垂直方向
        mLayoutManager.setOrientation(OrientationHelper.VERTICAL);
        //给RecyclerView设置布局管理器
        recyclerSupplies.setLayoutManager(mLayoutManager);
        arrayList = new ArrayList<>();
        adapter = new SuppliesAdapter(getActivity(), arrayList);
        recyclerSupplies.setAdapter(adapter);
        for (int i = 0; i < 2; i++) {
    ...
    protected void setActionBar() {
        ...
        activity.textTop.setText(R.string.my_supplies);
    }
